...
for i in range(n):
    ...
    PPT_norm    = scale(np.log(PPTtrain[i]))
    DPB_norm    = scale(DBPtrain[i])     
    SBP_norm    = scale(SBPtrain[i])
    
    idx = np.ones(np.shape(PPT_norm), dtype=bool)
    idx = np.where(abs(PPT_norm)<2,idx,False)   
    idx = np.where(abs(DPB_norm)<2,idx,False)
    idx = np.where(abs(SBP_norm)<2,idx,False)
    
    
    X = np.transpose(np.array([PPT_norm[idx]]))
    y1  = SBPtrain[i][idx]
    y2  = DBPtrain[i][idx]
...
plt.title('Simple Logarithmic Regression')
plt.show()
