...
#            PPT2 = (t_RR1[1: ]-t_PPG)
#            try:
#                t_HR.append(np.mean(HR))
#
#                if np.mean(PPT1)>np.mean(PPT2):
#                    t_PPT.append(np.mean(PPT1))
#                else:
#                    t_PPT.append(np.mean(PPT2))
#                    
#                t_DBP.append(DBP[cont])
#                t_SBP.append(SBP[cont])
#                cont +=1
#    #            plt.figure()
...
#
#        except:
#            print('error dato %s %d'%(nombres[k], cont+1))
##            t_HR.append([])
##            t_PPT.append([])
#
#        idx_a = j
...
HR_norm     = scale(t_HR)
logPPT_norm = scale(np.log(t_PPT))
DPB_norm    = scale(t_DBP)     
SBP_norm    = scale(t_SBP)
...
y1  = np.array(t_DBP)[idx]
y2  = np.array(t_SBP)[idx]

plt.figure()
sns.set_style("whitegrid")
sns.jointplot(X[:,1], y2, kind ='kde')
plt.xlabel('PPT') # Set text for the x axis
